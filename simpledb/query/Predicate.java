...
public class Predicate {
   ...
   private ArrayList<Integer> leftBrac = new ArrayList<Integer>();
   private ArrayList<Integer> rightBrac = new ArrayList<Integer>();
   private ArrayList<Integer> logicalOper = new ArrayList<Integer>();
   ...
   public void setComp(ArrayList<Integer> lb, ArrayList<Integer> rb, 
                       ArrayList<Integer> lo){
      leftBrac = lb;
      rightBrac = rb;
      logicalOper = lo;
      isComplex = true;
   ...
   public boolean isSatisfied(Scan s) {
      if (!isComplex)
         return simpleIsSatisfied(s);
      else
         return complexIsSatisfied(s);
   } 
   
   public boolean simpleIsSatisfied(Scan s) {
      for (Term t : terms)
         if (!t.isSatisfied(s))
         return false;
      return true;
   ...
   public boolean complexIsSatisfied(Scan s) {
      ArrayList<Object> stack1 = new ArrayList<Object>();
      ArrayList<Object> stack2 = new ArrayList<Object>();
      for (int i=0; i < terms.size(); i++) {
         if (i > 0)
            stack1.add(logicalOper.get(i-1));
         for (int j=0; j<leftBrac.get(i); j++)
            stack1.add("(");
         stack1.add(new Boolean(terms.get(i).isSatisfied(s)));
         for (int k=0; k<rightBrac.get(i); k++) {
            while (true) {
               Object e = stack1.remove(stack1.size()-1);
               if (e instanceof String)
                  break;
               else 
                  stack2.add(e);
            }
            Boolean b = (Boolean)stack2.remove(stack2.size()-1);
            while (stack2.size()>0) {
               Integer op = (Integer)stack2.remove(stack2.size()-1);
               Boolean b2 = (Boolean)stack2.remove(stack2.size()-1);
               if (op == 1)
                  b = b && b2;
               else
                  b = b || b2;
