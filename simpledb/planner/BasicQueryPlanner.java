...
public class BasicQueryPlanner implements QueryPlanner {
   ...
   public Plan createPlan(QueryData data, Transaction tx) {
      ...
      // Step 4 group by
      if (!data.groupFields().isEmpty() || !data.aggFns().isEmpty()) {
         p = new GroupByPlan(p, data.groupFields(), data.aggFns(), tx);
      }
      // Step 5 order by
      if(!data.sortFields().isEmpty()) {
          List<String> sortfields;
          sortfields = new ArrayList(data.sortFields());
          List<Boolean> desc;
          desc = new ArrayList(data.desc());
         p = new OrderPlan(p, sortfields, desc, tx);
      }
